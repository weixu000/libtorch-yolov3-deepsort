...
bool ImGui::FocusableItemRegister(ImGuiWindow *window, ImGuiID id, bool tab_stop) {
    ...
    if (tab_stop && (g.ActiveId == id) && window->FocusIdxAllRequestNext == INT_MAX &&
        ...
        window->FocusIdxTabRequestNext = window->FocusIdxTabCounter + (g.IO.KeyShift ? (is_tab_stop ? -1 : 0)
...
bool ImGui::Begin(const char *name, bool *p_open, ImGuiWindowFlags flags) {
    ...
    if (first_begin_of_the_frame) {
        ...
        if (!(flags & ImGuiWindowFlags_NoTitleBar)) {
            ...
            float marker_size_x = (flags & ImGuiWindowFlags_UnsavedDocument) ? CalcTextSize(UNSAVED_DOCUMENT_MARKER,
                                                                                            NULL, false).x : 0.0f;
            ...
            if (flags & ImGuiWindowFlags_UnsavedDocument) {
                ImVec2 marker_pos = ImVec2(ImMax(text_r.Min.x, text_r.Min.x + (text_r.GetWidth() - text_size.x) *
                                                                              style.WindowTitleAlign.x) + text_size.x,
                                           text_r.Min.y) + ImVec2(2 - marker_size_x, 0.0f);
                ImVec2 off = ImVec2(0.0f, (float) (int) (-g.FontSize * 0.25f));
                RenderTextClipped(marker_pos + off, text_r.Max + off, UNSAVED_DOCUMENT_MARKER, NULL, NULL,
...
static void ImGui::NavUpdateWindowing() {
    ...
    if (apply_toggle_layer && g.NavWindow) {
        ...
        g.NavDisableMouseHover = true;
        NavRestoreLayer(
                (g.NavWindow->DC.NavLayerActiveMask & (1 << ImGuiNavLayer_Menu)) ? (ImGuiNavLayer) ((int) g.NavLayer ^
                                                                                                    1)
                                                                                 : ImGuiNavLayer_Main);
    }
}
                    ...
                    for (int prim = clipper.DisplayStart, idx_i = elem_offset + clipper.DisplayStart * 3;
                        ...
                        for (int n = 0; n < 3; n++, idx_i++) {
                            ...
                            buf_p += ImFormatString(buf_p, buf_end - buf_p,
                                                    ...
                                                    (n == 0) ? "idx" : "   ", idx_i, v.pos.x, v.pos.y, v.uv.x, v.uv.y,
                                                    v.col);
                        }
