...
bool ImGui::CloseButton(ImGuiID id, const ImVec2 &pos, float radius) {
    ...
    if (hovered)
        window->DrawList->AddCircleFilled(center, ImMax(2.0f, radius),
                                          GetColorU32(held ? ImGuiCol_ButtonActive : ImGuiCol_ButtonHovered), 9);
    ...
    switch (data_type) {
        case ImGuiDataType_S8: {
            ...
            bool r = DragBehaviorT<ImS32, ImS32, float>(ImGuiDataType_S32, &v32, v_speed,
                                                        ...
                                                        v_max ? *(const ImS8 *) v_max : IM_S8_MAX, format, power,
                                                        flags);
        ...
        case ImGuiDataType_U8: {
            ...
            bool r = DragBehaviorT<ImU32, ImS32, float>(ImGuiDataType_U32, &v32, v_speed,
                                                        ...
                                                        v_max ? *(const ImU8 *) v_max : IM_U8_MAX, format, power,
                                                        flags);
...
bool ImGui::InputTextEx(const char *label, char *buf, int buf_size, const ImVec2 &size_arg, ImGuiInputTextFlags flags,
    ...
    if (is_readonly && state != NULL) {
        ...
        const bool will_render_selection =
                state->HasSelection() && (RENDER_SELECTION_WHEN_INACTIVE || will_render_cursor);
        ...
        if (IsKeyPressedMap(ImGuiKey_LeftArrow)) {
            ...
            if (!is_multiline || (ctrl_enter_for_new_line && !io.KeyCtrl) || (!ctrl_enter_for_new_line && io.KeyCtrl)) {
                ...
                if (InputTextFilterCharacter(&c, flags, callback, callback_user_data))
                   ...
                   !io.KeyShift && !io.KeyAlt && !is_readonly) {
    ...
    if (render_cursor || render_selection) {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, GetColorU32(ImGuiCol_Text),
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos, GetColorU32(ImGuiCol_Text), buf_display,
...
bool ImGui::BeginTabBar(const char *str_id, ImGuiTabBarFlags flags) {
    ...
    ImRect tab_bar_bb = ImRect(window->DC.CursorPos.x, window->DC.CursorPos.y, window->InnerClipRect.Max.x,
...
static void ImGui::TabBarLayout(ImGuiTabBar *tab_bar) {
    ...
    if (width_excess > 0.0f && (tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyResizeDown)) {
        ...
        if (tab_bar->Tabs.Size > 1)
            ImQsort(width_sort_buffer.Data, (size_t) width_sort_buffer.Size, sizeof(ImGuiTabBarSortItem),
...
bool ImGui::TabItemLabelAndCloseButton(ImDrawList *draw_list, const ImRect &bb, ImGuiTabItemFlags flags,
    ...
    if (close_button_visible) {
        ...
        if (CloseButton(close_button_id, ImVec2(bb.Max.x - frame_padding.x - close_button_sz,
                                                bb.Min.y + frame_padding.y + close_button_sz), close_button_sz))
    ...
    if (label_size.x > text_ellipsis_clip_bb.GetWidth()) {
        ...
        float label_size_clipped_x = g.Font->CalcTextSizeA(g.FontSize,
                                                           ...
                                                           0.0f, label, label_display_end, &label_end).x;
        ...
        {
            label_end--;
            label_size_clipped_x -= g.Font->CalcTextSizeA(g.FontSize, FLT_MAX, 0.0f, label_end,
                                                          label_end + 1).x; // Ascii blanks are always 1 byte
        }
        RenderTextClippedEx(draw_list, text_pixel_clip_bb.Min, text_pixel_clip_bb.Max, label, label_end, &label_size,
                            ImVec2(0.0f, 0.0f));
        ...
        if (!close_button_visible && ellipsis_x + ellipsis_width <= bb.Max.x)
            RenderPixelEllipsis(draw_list, ImVec2(ellipsis_x, text_pixel_clip_bb.Min.y), ellipsis_dot_count,
                                GetColorU32(ImGuiCol_Text));
    } else {
        RenderTextClippedEx(draw_list, text_pixel_clip_bb.Min, text_pixel_clip_bb.Max, label, label_display_end,
